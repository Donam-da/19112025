...
namespace namm
{
    ...
    public partial class SelectCustomerView : UserControl
    {
        ...
        {
            string filter = txtSearchCustomer.Text.Replace("'", "''"); 
        ...
        private async void BtnEditCustomer_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    await LoadCustomersAsync();
                    dgCustomers.SelectedValue = customerId; 
                }
                catch (SqlException ex)
                {
        ...
        private async void BtnPay_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_selectedCustomer != null)
            {
                ...
                decimal finalAmount = _totalAmount * (1 - (discountPercent / 100m)); 
        ...
        private async Task ProcessPaymentAsync(int? customerId, string? guestCustomerCode, decimal finalAmount)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                updateBillCmd.Parameters.AddWithValue("@SubTotal", _totalAmount); 
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                const string query = @"
                    SELECT
                        ...
                        ISNULL(SUM(b.TotalAmount), 0) AS TotalSpent
                    FROM Bill b
                    WHERE b.IdCustomer = @CustomerId AND b.Status = 1"; 
                var command = new SqlCommand(query, connection);
